...
public:
	...
	bool is_empty()
	{
		...
		int clip = *(int*)((DWORD)this + (netvar_manager::get_net_var(fnv::hash("DT_BaseCombatWeapon"),fnv::hash("m_iClip1"))));
	...
	bool is_reloading()
	{
		...
		auto type = get_weapon_data()->weapon_type;

		if (type == WEAPONTYPE_GRENADE || type == WEAPONTYPE_KNIFE)
			return false;
...
public:
	...
	bool physics_run_think(int index) {
		static auto physics_run_think_fn = reinterpret_cast<bool(__thiscall*)(void*, int)>(utilities::pattern_scan("client_panorama.dll", "55 8B EC 83 EC 10 53 56 57 8B F9 8B 87"));
